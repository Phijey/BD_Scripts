...
BEGIN
    ...
    ELSEIF NOT EXISTS (SELECT 1 FROM EMPLOYEES WHERE EMPLOYEENUMBER = PARAM_IDVENDEDOR AND JOBTITLE = 'SALES REP') THEN
        SET RESULTADO_DO_ITEM_PEDIDO = 'VENDEDOR NÃO ENCONTRADO NA BASE DE DADOS';
    ELSEIF NOT EXISTS (SELECT 1 FROM CARRINHO_DE_COMPRA WHERE IDCLIENTE = PARAM_IDCLIENTE) THEN
